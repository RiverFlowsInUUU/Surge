...
- 📚 **首页下沉规则集信息，新开专题接住** —— 首页原有三处规则集层面的内容：「📋 规则顺序」表（`surge-white-guard.list` / `AI.list` / `direct.txt` / `GEOIP,CN` 等）、「分流版的应用规则」表（13 行「规则集 → 去向」）与「📚 规则来源」整节。规则集属**实现侧**，读者只关心「能实现怎样的分流」，于是：
  ...
  - 📚 「规则来源」整节撤下首页；
  - 🆕 新开 [`docs/12-规则集与来源.md`](docs/12-规则集与来源.md) —— 10 份共用规则集 + 13 条分流版专有规则集的「作用 / 去向 / 来源」、两版逐条匹配顺序（含参数）、四条排序约束、素材与许可；
...
- 🔗 **修两处因首页下沉而失效的仓内指路** —— README 章节改名与撤下会让引用它的链接**静默失效**（GitHub 不报错，读者点 404），本轮由 `check_links.py` 抓出：`docs/04` §5 的 `../README.md#-规则顺序` → `#-分流顺序`；`docs/10` 的「规则集来源清单见 README 的 📚 规则来源」→ 改指 [`docs/12`](docs/12-规则集与来源.md)。
