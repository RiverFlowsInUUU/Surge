...
> 📌 **该坑已随架构变更消失**（配置已收敛为单一版本，见
> [`docs/07`](../../docs/07-文件版本沿革.md) §3）。**但教训仍然成立**，
> 留着防止下次又冒出"精简版"。

**症状**（当时）：架构检查报「`v0.conf` 没有白名单守卫」。
...
`skill/scripts/audit_region_filters.py` 做三条**绝对判据**（不是模糊匹配）：
...
`skill/tests/fixtures/bad_region_filter.conf` 刻意把 Hong Kong 组的关键词
