...
| 14 | `proxy-test-url` 用境外端点 → 持续泄露面 | 每 5 分钟一次的境外解析 |
...
⇒ 这也是「`AD` 组不由任何规则引用」的根因。

**连带坑**：把 `AD` 组切成 `DIRECT` **不会**关闭广告拦截 ——
规则根本不经过它。想真正关掉，改规则那一行的策略或注释掉整行。
...
## 14 · `proxy-test-url` 用境外端点 → 持续泄露面

**症状**：抓包发现每 5 分钟一次境外域名解析。

**根因**：`proxy-test-url = http://www.gstatic.com/generate_204`。
这个 URL **每个策略组每 5 分钟跑一次**，是持续性动作。

**修法**：换国内 204（`http://connect.rom.miui.com/generate_204`），
让解析被 `direct.txt` 接住走直连；且国内端点 RTT 方差小，
测出的分数更能反映**节点本身**而不是链路抖动。
